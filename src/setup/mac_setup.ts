...
async function getDownloadURL(swiftVersion: string, platformName: string, pkgName: string) {
...
async function unpack(downloadPath: string, pkgName: string) {
    ...
    const extractPath = await toolCache.extractTar(path.join(unpackedPath, `${pkgName}-package.pkg`, 'Payload'));
    return extractPath;
}